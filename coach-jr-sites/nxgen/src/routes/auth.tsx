...
const PROVIDER_LABEL: Record<string, string> = { google: "Google", apple: "Apple", facebook: "Facebook" };

// Show a social button only once its provider is actually enabled in Supabase
// (Auth → Providers). Flip the matching Vercel env var to "true" then redeploy:
//   VITE_ENABLE_GOOGLE / VITE_ENABLE_APPLE / VITE_ENABLE_FACEBOOK
// Keeps unconfigured buttons hidden so they can't throw "provider is not enabled".
const env = (typeof import.meta !== "undefined" && import.meta.env) || ({} as Record<string, string>);
// Google is configured in Supabase (2026-09-19), so it's on by default. Apple/Facebook
// stay env-gated until their providers are set up.
const ENABLE_GOOGLE = env.VITE_ENABLE_GOOGLE !== "false";
const ENABLE_APPLE = env.VITE_ENABLE_APPLE === "true";
const ENABLE_FACEBOOK = env.VITE_ENABLE_FACEBOOK === "true";
const ANY_OAUTH = ENABLE_GOOGLE || ENABLE_APPLE || ENABLE_FACEBOOK;
...
function AuthPage() {
  ...
  const handleOAuth = async (provider: "google" | "apple" | "facebook") => {
    setLoading(true);
    const result = await oauth.auth.signInWithOAuth(provider, { redirect_uri: authReturnUrl() });
    if (result.error) {
      setLoading(false);
              ...
              <div className="grid gap-3">
                {ENABLE_GOOGLE && (
                  <Button onClick={() => handleOAuth("google")} disabled={loading} variant="outline" size="lg">
                    <svg viewBox="0 0 24 24" className="mr-2 h-4 w-4" aria-hidden><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23z"/><path fill="#FBBC05" d="M5.84 14.1a6.6 6.6 0 0 1 0-4.2V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.84z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 7.31 9.14 5.38 12 5.38z"/></svg>
                    Continue with Google
                  </Button>
                )}
                {ENABLE_FACEBOOK && (
                  <Button onClick={() => handleOAuth("facebook")} disabled={loading} variant="outline" size="lg">
                    <svg viewBox="0 0 24 24" className="mr-2 h-4 w-4" fill="#1877F2" aria-hidden><path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8V24C19.61 23.1 24 18.1 24 12.07z"/></svg>
