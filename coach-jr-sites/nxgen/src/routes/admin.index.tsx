...
/** Shortcut tiles: the highest-frequency admin jobs. `count` reads a live
 * pending number off the dashboard snapshot so a task with a queue shows a
 * badge and the quiet ones stay clean. */
const QUICK: {
  to: string;
  label: string;
  icon: React.ComponentType<{ className?: string }>;
  count?: (t: Today) => number;
  adminOnly?: boolean;
}[] = [
  { to: "/admin/score", label: "Score Entry", icon: Smartphone, count: (t) => t.games.length },
  { to: "/admin/approvals", label: "Approvals", icon: ClipboardList },
  { to: "/admin/payments", label: "Payments", icon: Receipt, count: (t) => t.pendingPayments, adminOnly: true },
  { to: "/admin/roster", label: "Players", icon: Users },
  { to: "/admin/schedule", label: "Schedule", icon: CalendarRange },
  { to: "/admin/gallery", label: "Photos", icon: ImageIcon },
];
...
type Today = {
  loading: boolean;
  /** Set when a lookup failed, so a zero is never mistaken for "all clear". */
  error: string | null;
...
function useToday(): Today {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!mounted) return;
      // Every one of these degrades to 0 or [] on failure, and the dashboard
      // then reports "nothing pending" — including the count of minors with no
      // guardian consent on file, which is the one number on this screen that
      // must never quietly read zero.
      const failed = [games.error, payments.error, waivers.error, teams.error, players.error, refunds.error]
        .filter(Boolean)
        .map((e) => e!.message);
      setState({
        loading: false,
  ...
  return (
    <div className="adm-root">
      <div className="adm-wrap">
        ...
        {/* QUICK ACTIONS — the jobs done most nights, one tap away, with live
            pending counts so the busy ones announce themselves. Chosen for
            frequency: score entry, approvals, payments, players, schedule,
            photos. Admin-only tasks drop out for staff. */}
        <div className="qa-wrap">
          <p className="qa-eyebrow">Quick Actions</p>
          <div className="qa-bar">
            {QUICK.filter((q) => !q.adminOnly || isAdmin).map((q) => {
              const count = q.count ? q.count(t) : 0;
              return (
                <Link key={q.to} to={q.to} className="qa-tile">
                  <span className="qa-ic"><q.icon className="h-5 w-5" /></span>
                  <span className="qa-lb">{q.label}</span>
                  {!t.loading && count > 0 && <span className="qa-badge">{count}</span>}
                </Link>
              );
            })}
          </div>
        ...
        <div className="kpi-strip">
          ...
          <div className="kpi kpi-red">
            ...
            <div className="kpi-val mono">{t.loading ? "—" : t.error ? "?" : t.pendingPayments}</div>
        ...
        <div className="alert-strip">
          ...
          {t.loading ? (
            ...
            <div className="a-row">
              {t.error
                ? "Could not check — this list may be incomplete."
                : "Everything in the last/next 7 days is final."}
            </div>
          ) : (
