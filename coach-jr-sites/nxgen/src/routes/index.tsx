...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: `${SITE_URL}${nxgLogo.url}` },
      { name: "twitter:image", content: `${SITE_URL}${nxgLogo.url}` },
    ],
  ...
  return (
    <section id="contact" className="section" style={{ background: "var(--s0)" }}>
      <div className="container">
        ...
        <div className="r3 d3" style={{ marginTop: 20, display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center" }}>
  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="hero">
          ...
          <div className="hero-content">
            ...
            <div className="hero-ctas hero-line" style={{ animationDelay: "1.14s" }}>
              <Link to="/register" search={{ as: "solo" }} className="btn btn-gold">Sign Up Free &#8594;</Link>
              <a href="#divisions" className="btn btn-ghost">Explore Divisions</a>
            </div>
            {/* Team registration is a coaches' path — moved out of the primary
                CTA so the individual sign-up funnel leads. */}
            <p className="hero-coach hero-line" style={{ animationDelay: "1.22s" }}>
              Coaching a team? <Link to="/register" search={{ as: "team" }}>Register your squad &#8594;</Link>
            </p>
          </div>
        ...
        {settings.sections.contact && <Contact />}
        ...
        <section id="about" className="section" style={{ padding: "80px 0", background: "var(--s0)" }}>
          <div className="container">
            ...
            <p className="r3 d2" style={{ color: "var(--silver-d)", maxWidth: 640, margin: "16px auto 0", lineHeight: 1.8, fontSize: 14, textAlign: "center" }}>
              {settings.about_text}
            </p>
            {/* Facts pulled out of the paragraph so they read as a record, not a
                run-on sentence. */}
            <dl className="r3 d3" style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "18px 40px", margin: "28px auto 0", maxWidth: 640, borderTop: "1px solid var(--line)", paddingTop: 24 }}>
              {[
                ["Commissioner", "Coach Jr"],
                ["Year Founded", "2026"],
                ["Official Venue", `${settings.venue_name}, ${settings.venue_location}`],
              ].map(([k, v]) => (
                <div key={k} style={{ textAlign: "center", minWidth: 0 }}>
                  <dt style={{ fontFamily: "var(--fm)", fontSize: 10, letterSpacing: ".16em", textTransform: "uppercase", color: "var(--silver-d)", marginBottom: 6 }}>{k}</dt>
                  <dd style={{ margin: 0, fontFamily: "var(--fd)", fontWeight: 800, fontSize: 15, color: "var(--paint)" }}>{v}</dd>
                </div>
              ))}
            </dl>
          </div>
