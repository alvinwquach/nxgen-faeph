...
export const Route = createFileRoute("/register")({
  ...
  // ?as=solo|team preselects the tab so the hero's "Sign Up" opens the
  // individual form and the coaches' link opens team registration.
  validateSearch: (s: Record<string, unknown>): { as?: "team" | "solo" } =>
    s.as === "solo" || s.as === "team" ? { as: s.as } : {},
  component: RegisterPage,
});

/** Division label on this form -> the key its fee is stored under. */
...
function RegisterPage() {
  ...
  // Divisions the admin has switched off drop out of the pickers, and the
  // master switch closes the form outright. Both were previously settings that
  // saved and did nothing.
  const isOpen = (key: string) => settings.division_open[key] !== false;
  const { as } = Route.useSearch();
  const [tab, setTab] = useState<"team" | "solo">(as ?? "team");
    ...
    (async () => {
      try {
        ...
        if (raw && !autoRan.current) {
          ...
          if (draft.tab && !as) setTab(draft.tab); // URL intent wins over a saved draft
  ...
  async function submitRow(kind: "team" | "solo", t: TeamForm, s: SoloForm, userId: string) {
    ...
    try {
      ...
      setDone({ division, fee: settings.division_fees[FEE_KEY[division]] ?? 0 });
  ...
  // The master switch in Site Settings now actually closes the form.
  if (!settings.registration_open) {
    return (
      <NxPage eyebrow="Season 2026" title="Registration closed">
        <p style={{ color: "var(--silver-d)", fontSize: 14, lineHeight: 1.8, maxWidth: 560 }}>
          Entries are closed for now. Follow the league on{" "}
          <a href={settings.social_facebook} target="_blank" rel="noreferrer" style={{ color: "var(--gold-l)" }}>Facebook</a>{" "}
          and we will post as soon as the next intake opens.
        </p>
      </NxPage>
    );
  }
  ...
  return (
    <NxPage
      eyebrow="Season 2026"
      title="Register"
      intro={`Lock your spot at ${settings.venue_name}, ${settings.venue_location.split(",")[0]}. All fields are required unless marked optional.`}
    >
          ...
          <>
            <Field label="Division">
              <select required style={fieldStyle} value={team.division} onChange={(e) => setTeam({ ...team, division: e.target.value })}>
                ...
                {isOpen("rising-stars") && <option value="Rising Stars">Rising Stars — {peso(settings.division_fees["rising-stars"] ?? 0)}</option>}
                {isOpen("legacy") && <option value="Legacy">Legacy — {peso(settings.division_fees.legacy ?? 0)}</option>}
                {isOpen("3x3") && <option value="3x3">3x3 — {peso(settings.division_fees["3x3"] ?? 0)}</option>}
              </select>
            </Field>
          ...
          <>
            <Field label="Division">
              <select required style={fieldStyle} value={solo.division} onChange={(e) => setSolo({ ...solo, division: e.target.value })}>
                <option value="">Select a division</option>
                {isOpen("kotc") && <option value="King of the Court">King of the Court — {peso(settings.division_fees.kotc ?? 0)}</option>}
                {isOpen("3x3") && <option value="3x3 Individual">3x3 Individual — {peso(settings.division_fees["3x3"] ?? 0)}</option>}
              </select>
            </Field>
        ...
        {(() => {
          ...
          return (
            <div style={{ margin: "4px 0 18px", padding: "12px 14px", borderRadius: 10, background: "rgba(201,162,39,.08)", border: "1px solid var(--line-g)" }}>
              ...
              <span className="mono" style={{ color: "var(--gold)", fontSize: 16, fontWeight: 700 }}>{peso(settings.division_fees[FEE_KEY[div]] ?? 0)}</span>
            </div>
          );
        })()}
          ...
          <div style={{ marginBottom: 18, padding: 14, borderRadius: 10, border: "1px solid var(--line-g)", background: "var(--s2)" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {import.meta.env?.VITE_ENABLE_FACEBOOK === "true" && (
                <button type="button" className="btn btn-ghost btn-sm" onClick={() => signIn("facebook")}>Continue with Facebook</button>
              )}
              ...
              <a className="btn btn-ghost btn-sm" href="/auth?redirect=/register">Sign in with email</a>
            </div>
          </div>
        )}
