...
import { getPublicPlayerNames } from "@/lib/public-players.functions";
import {
...
export type PlayerSeason = {
  ...
  peers: ComparePeer[];
  /** Strongest badge per family, for the back of the card. */
  cardBadges: CardBadge[];
  earnedCount: number;
  achievementCtx: { lines: GameLine[]; summary: SeasonSummary };
};

/**
 * One season fetch, shared by everything that draws from it.
 *
 * Pulled out of the hub so the account page can show the same card, built from
 * the same numbers, without mounting the whole tabbed console.
 */
export function usePlayerSeason(playerId: string, division?: string | null): PlayerSeason {
  ...
  const fetchPublicNames = useServerFn(getPublicPlayerNames);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (div) {
        ...
        if (ids.length) {
          // Names come from a server lookup: the profiles table is not readable
          // by signed-out visitors, and the lookup only returns public profiles.
          const profs = await fetchPublicNames({ data: { ids } }).catch(() => []);
          if (cancelled) return;
  ...
  }, [playerId, division, fetchPublicNames]);
  ...
  return { loading, lines, summary, peers, cardBadges, earnedCount, achievementCtx };
}
...
/**
 * The trading card on its own, loading its own season.
 *
 * For places that want the card and nothing else — the account dashboard — so
 * they get the same rating, radar and badges as the profile rather than a
 * simpler second card that drifts out of step with it.
 */
export function NxPlayerCardLive(props: CardIdentity) {
  const { loading, summary, cardBadges } = usePlayerSeason(props.playerId, props.division);

  if (loading) {
    return <div className="hub-loading" role="status">Loading your card…</div>;
  }
...
export function NxPlayerHub({
  ...
  const { loading, summary, peers, cardBadges, earnedCount, achievementCtx } =
    usePlayerSeason(playerId, division);
