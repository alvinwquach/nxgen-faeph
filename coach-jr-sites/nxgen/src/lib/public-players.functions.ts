...
/** Names of players whose profiles are public — safe for anonymous visitors. */
export const getPublicPlayerNames = createServerFn({ method: "GET" })
  .inputValidator((data) => z.object({ ids: z.array(z.string().uuid()).max(500) }).parse(data))
  .handler(async ({ data }) => {
    if (!data.ids.length) return [] as Array<{ id: string; full_name: string }>;
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: rows, error } = await supabaseAdmin
      .from("profiles")
      .select("id, full_name, is_public, consent_status")
      .in("id", data.ids)
      .eq("is_public", true);
    if (error) throw new Error(error.message);
    return (rows ?? [])
      .filter((r) => r.full_name && r.consent_status !== "pending")
