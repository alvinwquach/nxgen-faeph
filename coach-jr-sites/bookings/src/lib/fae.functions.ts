...
import { PEAK, SPORTS, isMember, type SportKey } from "./constants";
...
/** Public — the booking page and landing both price from this. */
export const getCourtRates = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await supabasePublic.from("court_rates").select("*").eq("active", true).order("sort_order");
  if (error) throw new Error(error.message);
  return { rates: (data ?? []) as CourtRate[] };
});

export const updateCourtRate = createServerFn({ method: "POST" })
  .inputValidator((input) =>
    z
      .object({
        courtId: z.string().min(1),
        memberRate: z.number().min(0).max(100000),
        nonMemberRate: z.number().min(0).max(100000),
      })
      .parse(input),
  )
  .middleware([requireSupabaseAuth])
  .handler(async ({ data, context }) => {
    ...
    const { data: prev } = await supabase
      ...
      .maybeSingle();
    if (!prev) throw new Error("That court is not in the rate table.");

    const { error } = await supabase
      .from("court_rates")
      .update({
        member_rate: data.memberRate,
        non_member_rate: data.nonMemberRate,
    ...
    await supabase.from("activity_log").insert({
      action: `Court rate changed · ${prev.name}`,
      details: data.courtId,
      actor_id: userId,
      actor_name: actorName,
      before_after: `member ₱${prev.member_rate} → ₱${data.memberRate} · non-member ₱${prev.non_member_rate} → ₱${data.nonMemberRate}`,
    });
...
export const createBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Price from the rate table so counter edits take effect immediately.
    // Falls back to the compiled constants if the court has no row yet.
    const { data: rateRow } = await supabase
      ...
      .eq("active", true)
      .maybeSingle();
    const memberTier = isMember(member.tier);
    const dbRate = rateRow ? Number(memberTier ? rateRow.member_rate : rateRow.non_member_rate) : null;
    ...
    for (const block of blocks) {
      const amount =
        dbRate !== null
          ? // Mirror hourRate(): the peak uplift has to apply to the DB rate too,
            // or turning PEAK on later would undercharge while the UI still shows it.
            block.reduce(
              (sum, h) => sum + Math.round(dbRate * (PEAK.enabled && h >= PEAK.start ? 1 + PEAK.uplift : 1)),
              0,
            )
          : priceHours(data.sport as SportKey, data.courtId, block, memberTier);
