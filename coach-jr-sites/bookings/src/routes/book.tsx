...
export const Route = createFileRoute("/book")({
  ...
  head: () => ({
    meta: [
      { title: "Book a court by the hour — FAE Bookings" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Book a court by the hour — FAE Bookings" },
      {
...
function BookPage() {
  ...
  const { data: rateData } = useQuery({ queryKey: ['court-rates'], queryFn: () => getCourtRates({ data: undefined }) });
  /** Live price per hour for a court, falling back to the compiled rate. */
  const rateFor = (id: string, member: boolean) => {
    const row = rateData?.rates.find((r) => r.court_id === id);
    if (row) return Number(member ? row.member_rate : row.non_member_rate);
    const c = SPORT_KEYS.flatMap((k) => SPORTS[k].courts).find((x) => x.id === id);
    return c ? (member ? c.memberRate : c.nonMemberRate) : 0;
  };

  const selectedHours = useMemo(() => [...selected].sort((a, b) => a - b), [selected]);
  const isMember = tierIsMember(profile?.member?.tier);
  const subtotal = selectedHours.reduce(
    (sum, h) => sum + Math.round(rateFor(courtId, isMember) * (PEAK.enabled && h >= PEAK.start ? 1 + PEAK.uplift : 1)),
    0,
  );
  ...
  const reserve = async () => {
    ...
    try {
      ...
      // Free up any unpaid hold whose grace window lapsed, so a paying member can take the slot.
      const firstHour = Math.min(...selectedHours);
      await claimLapsedHolds({
        data: { courtId, date, startHour: firstHour, hours: Math.max(...selectedHours) - firstHour + 1 },
      });
          ...
          <div className="flex flex-wrap gap-2.5">
            {SPORTS[sport].courts.map((c) => {
              ...
              return (
                ...
                >
                  {c.name} <span className="ml-1 font-mono text-xs text-gold">· {formatPeso(rateFor(c.id, isMember))}/hr {isMember ? "member" : "non-member"}</span>
                </button>
        ...
        {confirmation ? (
          <div>
            ...
            <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
              50% deposit by GCash, Maya or bank transfer reserves the slot — send the proof from My Account right
              after booking. The remaining half is due on arrival, cash or e-wallet. Free cancellation up to 6 hours
              before your slot.
            </p>
