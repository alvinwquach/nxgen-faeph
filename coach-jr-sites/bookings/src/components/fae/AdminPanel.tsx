...
import { BookingRequests } from "@/components/fae/BookingRequests";
...
import { getBookingRequests } from "@/lib/schedule.functions";
import type { AdminData, BookingRow, InventoryRow, MemberRow, SaleRow, TabItem, TabRow } from "@/lib/fae.types";

/* ---------- Shared helpers ---------- */

type AdminTab = "overview" | "bookings" | "requests" | "schedule" | "inventory" | "clients" | "tabs" | "wifi" | "payments" | "settings";

const ADMIN_TABS: { key: AdminTab; label: string; icon: string }[] = [
  { key: "overview", label: "Overview", icon: "chart" },
  { key: "bookings", label: "Bookings", icon: "calendar" },
  { key: "requests", label: "Requests", icon: "calendar-clock" },
...
function memberName(data: AdminData, memberId: string | null, fallback?: string | null): string {
  return data.members.find((m) => m.id === memberId)?.name ?? (fallback || "Walk-in");
}
...
// Covers every status the booking flows write (Paid, Paid - Cash/GCash/Other, Booked, Confirmed, Reserved…).
function statusBadge(status: string | null): { label: string; variant: BadgeVariant } {
  ...
  if (status?.startsWith("Paid") || status === "Booked" || status === "Confirmed") return { label: status === "Confirmed" ? "Confirmed" : "Paid", variant: "gold" };
  if (status === "Reserved") return { label: "Reserved", variant: "blue" };
  if (status === "Unpaid") return { label: "Unpaid", variant: "red" };
...
function AdminNote({ children }: { children: ReactNode }) {
  return (
    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 pt-16">
      <div className="max-w-md rounded-xl border border-border bg-surface-2 p-10 text-center">
        ...
        <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
          This panel is restricted to F.A.E. staff. If you run the desk, ask an existing admin to add your account.
        </p>
      </div>
    </main>
  );
...
/** Own component so the whole panel doesn't re-render every second. */
function LiveClock() {
...
/** Counter + quick-book modals shared by both admin layouts. */
function useAdminModals(data: AdminData | undefined) {
  ...
  return { refresh, setSaleItem, setRestock, setAddItemOpen, setAddClientOpen, setPicker, setQuickBook, modals };
}

/** Classic 10-tab panel, kept at /admin until the new layout is approved. */
export function AdminPage() {
  ...
  const { refresh, setSaleItem, setRestock, setAddItemOpen, setAddClientOpen, setPicker, setQuickBook, modals } =
    useAdminModals(data);

  if (statusLoading) return <AdminNote>Checking access…</AdminNote>;
  if (!status?.isAdmin) return <StaffOnly />;
  if (!data) return <AdminNote>Loading operations…</AdminNote>;
  ...
  return (
    <main className="min-h-screen bg-background pb-24">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        {tab === "requests" ? <BookingRequests /> : null}
...
/* ---------- Operations (v2): 6 tabs, Today first, desk staff get a limited view ---------- */

type OpsTab = "today" | "schedule" | "payments" | "wifi" | "counter" | "clients" | "settings";
type ScheduleView = "requests" | "board" | "week";
type CounterView = "sales" | "inventory" | "tabs";

const OPS_TABS: { key: OpsTab; label: string; icon: string; adminOnly?: boolean }[] = [
  { key: "today", label: "Today", icon: "chart" },
  { key: "schedule", label: "Schedule", icon: "calendar" },
  { key: "payments", label: "Payments", icon: "credit-card" },
  { key: "wifi", label: "Internet & Data", icon: "wifi" },
  { key: "counter", label: "Counter", icon: "receipt", adminOnly: true },
  { key: "clients", label: "Clients", icon: "users", adminOnly: true },
];

export function AdminV2() {
  const [tab, setTab] = useState<OpsTab>("today");
  const [scheduleView, setScheduleView] = useState<ScheduleView>("requests");
  const [counterView, setCounterView] = useState<CounterView>("sales");
  ...
  // Desk staff: read-only role check, same gate the staff server functions use.
  const { data: isStaff, isLoading: staffLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: ok } = await supabase.rpc("has_staff_access", { _user_id: auth.user.id });
      return !!ok;
    },
  });
  ...
  // Same query keys as the panels below, so the counts share their cache instead of double-fetching.
  ...
  const { data: todaySched } = useQuery({
    queryKey: ["schedule-today", todayIso],
    queryFn: () => getSchedule({ data: { start: todayIso, days: 1 } }),
    enabled: canUse,
    refetchInterval: 60_000,
  });
  ...
  if (statusLoading || (!isAdmin && staffLoading)) return <AdminNote>Checking access…</AdminNote>;
  if (!canUse) return <StaffOnly />;
  if (isAdmin && !data) return <AdminNote>Loading operations…</AdminNote>;

  const requests = req?.requests ?? [];
  const counts = {
    pending: requests.filter((r) => r.status === "Pending").length,
    reserved: requests.filter((r) => r.status === "Reserved").length,
    toVerify: (pay?.payments ?? []).filter((p) => p.status === "Submitted").length,
    wifiWaiting: (wifi?.orders ?? []).filter((o) => o.status === "Pending" || o.status === "Paid").length,
    lowStock: data ? data.inventory.filter((i) => i.stock != null && i.stock <= (i.par_level ?? 5)).length : 0,
  };
  const badges: Partial<Record<OpsTab, number>> = {
    schedule: counts.pending,
    payments: counts.toVerify,
    wifi: counts.wifiWaiting,
    counter: counts.lowStock,
  };
  const tabs = OPS_TABS.filter((t) => isAdmin || !t.adminOnly);

  const go = (next: OpsTab, sub?: ScheduleView | CounterView) => {
    setTab(next);
    if (next === "schedule" && sub) setScheduleView(sub as ScheduleView);
    if (next === "counter" && sub) setCounterView(sub as CounterView);
  };
  ...
  return (
    <main className="min-h-screen bg-background pb-24">
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-5 sm:px-6">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold">
              {isAdmin ? "Admin" : "Desk staff"} · FAE Bookings
            </p>
            <h1 className="mt-1 font-display text-2xl font-black uppercase tracking-tight text-foreground">Operations</h1>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <LiveClock />
            {isAdmin ? (
              <Button variant="ghost" size="sm" aria-pressed={tab === "settings"} onClick={() => setTab("settings")}>
                <Icon name="activity" size={14} /> Settings
              </Button>
            ) : null}
            {isAdmin ? (
              <Link to="/admin" className="px-2 font-mono text-[10px] uppercase tracking-wide text-muted-foreground hover:text-foreground">
                Classic view
              </Link>
            ) : null}
          </div>
      ...
      <OpsTabBar tabs={tabs} active={tab} badges={badges} onChange={setTab} />

      <div id="ops-panel" role="tabpanel" aria-label={tab} className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        {tab === "today" ? (
          <TodayTab
            data={data}
            counts={counts}
            bookings={(todaySched?.bookings ?? []).filter((b) => b.status !== "Cancelled")}
            onGo={go}
            onLogSale={() => setPicker("sale")}
            onAddClient={() => setAddClientOpen(true)}
            onRestock={() => setPicker("restock")}
        ...
        {tab === "schedule" ? (
          <>
            <Segmented
              ...
              options={[
                { value: "requests", label: "Requests", count: counts.pending },
                { value: "board", label: "Court board" },
                ...(isAdmin ? [{ value: "week" as const, label: "All courts · week" }] : []),
              ]}
            />
            {scheduleView === "requests" ? <BookingRequests /> : null}
            {scheduleView === "board" ? <ScheduleBoard /> : null}
            {scheduleView === "week" && data ? (
              <BookingsTab data={data} onQuickBook={(date, hour) => setQuickBook({ date, hour })} />
            ) : null}
          </>
        ) : null}
          ...
          <>
            <Segmented
              ...
              options={[
                ...
                { value: "inventory", label: "Inventory", count: counts.lowStock },
                { value: "tabs", label: "Open tabs", count: data.openTabs.length },
              ]}
            />
            {counterView === "sales" ? (
              <div className="space-y-4">
                <Button variant="gold" size="sm" onClick={() => setPicker("sale")}>
                  <Icon name="receipt" size={14} /> Log sale
                </Button>
                <SalesLog data={data} />
              </div>
            ) : null}
            {counterView === "inventory" ? (
              <InventoryTab
                data={data}
                onSale={(item) => setSaleItem(item)}
                onRestock={(item) => setRestock(item)}
            ...
            {counterView === "tabs" ? <TabsTab data={data} onChanged={refresh} /> : null}
          </>
        ) : null}

        {tab === "clients" && data ? <ClientsTab data={data} onAddClient={() => setAddClientOpen(true)} /> : null}
        {tab === "settings" && isAdmin ? <SettingsTab /> : null}
      </div>
...
/** ARIA tablist: arrow keys / Home / End move between sections; badges show what needs attention. */
function OpsTabBar({
  tabs,
  active,
  badges,
        ...
        {tabs.map((t, i) => {
          ...
          return (
            <button
              ...
              tabIndex={(activeIdx === -1 ? i === 0 : selected) ? 0 : -1}
              onClick={() => onChange(t.key)}
              className={cn(
                "flex shrink-0 items-center gap-2 border-b-2 px-4 py-3.5 text-[13px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold",
                selected ? "border-gold text-foreground" : "border-transparent text-muted-foreground hover:text-foreground",
              )}
            >
              ...
              {n > 0 ? (
                <>
                  <span aria-hidden="true" className="min-w-5 rounded-full bg-gold px-1.5 text-center font-mono text-[10px] font-bold leading-5 text-background">
                    {n}
                  </span>
...
function Segmented<T extends string>({
  ...
  return (
    <div role="group" aria-label={label} className="mb-5 inline-flex flex-wrap gap-1 rounded-xl border border-border bg-surface-1 p-1">
      {options.map((o) => (
        <button
          key={o.value}
          type="button"
          ...
          className={cn(
            "flex items-center gap-2 rounded-lg px-3.5 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold",
            value === o.value ? "bg-surface-3 text-foreground ring-1 ring-goldline" : "text-muted-foreground hover:text-foreground",
          )}
        >
          {o.label}
          {o.count ? <span className="rounded-full bg-gold/20 px-1.5 font-mono text-[10px] text-gold">{o.count}</span> : null}
        </button>
      ))}
    </div>
  );
...
function ActionCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group rounded-xl border bg-surface-2 p-4 text-left transition-colors hover:border-goldline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold",
        urgent ? "border-goldline" : "border-border",
      )}
    >
      <span className="flex items-center justify-between">
        <Icon name={icon} size={16} className={urgent ? "text-gold" : "text-muted-foreground"} />
        <Icon name="arrow-right" size={14} className="text-muted-foreground transition-transform group-hover:translate-x-0.5" />
      </span>
      <span className={cn("mt-3 block font-display text-2xl font-extrabold", urgent ? "text-gold" : "text-foreground")}>{value}</span>
...
function TodayTab({
  ...
  counts: { pending: number; reserved: number; toVerify: number; wifiWaiting: number; lowStock: number };
  bookings: BookingRow[];
  onGo: (tab: OpsTab, sub?: ScheduleView | CounterView) => void;
  onLogSale: () => void;
  onAddClient: () => void;
  onRestock: () => void;
}) {
  const sorted = [...bookings].sort((a, b) => a.start_hour - b.start_hour);
  const hours = sorted.reduce((s, b) => s + b.hours, 0);
  const revenue = sorted.reduce((s, b) => s + Number(b.amount), 0);
  const salesToday = data ? data.todaySales.reduce((s, r) => s + r.amount, 0) : 0;
  const nowHour = new Date().getHours();

  return (
    <div className="space-y-6">
      <section aria-labelledby="needs-attention">
        <h2 id="needs-attention" className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
          Needs attention
        </h2>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <ActionCard
            ...
            hint={counts.reserved ? `${counts.reserved} reserved, awaiting payment` : "Approve or decline"}
            urgent={counts.pending > 0}
            onClick={() => onGo("schedule", "requests")}
          />
          <ActionCard
            icon="credit-card"
            value={counts.toVerify}
            label="Payments to verify"
            hint="Check GCash / bank proof"
            urgent={counts.toVerify > 0}
            onClick={() => onGo("payments")}
          />
          <ActionCard
            icon="wifi"
            value={counts.wifiWaiting}
            label="WiFi orders waiting"
            hint="Collect payment or switch on"
            urgent={counts.wifiWaiting > 0}
            onClick={() => onGo("wifi")}
          />
          <ActionCard
            icon="calendar"
            value={sorted.length}
            label="Court bookings today"
            hint={`${hours} hrs · ${formatPeso(revenue)}`}
            onClick={() => onGo("schedule", "board")}
          />
        </div>
      </section>

      {data ? (
        <section aria-label="Counter" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <ActionCard icon="receipt" value={formatPeso(salesToday)} label="Counter sales today" hint="Sales log" onClick={() => onGo("counter", "sales")} />
          <ActionCard icon="receipt" value={data.openTabs.length} label="Open tabs" hint="Settle or void items" onClick={() => onGo("counter", "tabs")} />
          <ActionCard
            icon="package"
            value={counts.lowStock}
            label="Low-stock items"
            hint="Restock soon"
            urgent={counts.lowStock > 0}
            onClick={() => onGo("counter", "inventory")}
          />
          <ActionCard icon="users" value={data.members.length} label="Members & clients" hint="Search clients" onClick={() => onGo("clients")} />
        </section>
      ) : null}

      <div className="grid gap-6 lg:grid-cols-3">
        <section className="rounded-xl border border-border bg-surface-2 p-4 lg:col-span-2" aria-labelledby="on-court">
          <div className="flex items-center justify-between gap-3">
            <h2 id="on-court" className="font-display text-sm font-extrabold uppercase tracking-wide text-foreground">
              Today on court
            </h2>
            <Button variant="ghost" size="sm" onClick={() => onGo("schedule", "board")}>
              Open court board <Icon name="arrow-right" size={12} />
            </Button>
          </div>
          <ul className="mt-3 divide-y divide-border/60">
            {sorted.map((b) => {
              const st = statusBadge(b.status);
              const live = nowHour >= b.start_hour && nowHour < b.start_hour + b.hours;
              return (
                <li key={b.id} className="flex items-center gap-3 py-2.5 text-sm">
                  <span className={cn("w-28 shrink-0 font-mono text-xs", live ? "text-gold" : "text-muted-foreground")}>
                    {formatHour(b.start_hour)}–{formatHour(b.start_hour + b.hours)}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-medium text-foreground">
                      {data ? memberName(data, b.member_id, b.booker_name) : b.booker_name || "Walk-in"}
                    </span>
                    <span className="block truncate text-xs text-muted-foreground">{courtLabel(b.sport, b.court_id)}</span>
                  </span>
                  {live ? <Badge variant="green">On court</Badge> : null}
                  <Badge variant={st.variant}>{st.label}</Badge>
                  <span className="hidden w-20 text-right font-mono text-xs text-foreground sm:block">{formatPeso(Number(b.amount))}</span>
                </li>
              );
            })}
            {sorted.length === 0 ? <li className="py-6 text-center text-sm text-muted-foreground">No court bookings today.</li> : null}
          </ul>
        </section>

        <section className="rounded-xl border border-border bg-surface-2 p-4" aria-labelledby="quick-actions">
          <h2 id="quick-actions" className="font-display text-sm font-extrabold uppercase tracking-wide text-foreground">
            Quick actions
          </h2>
          <div className="mt-3 space-y-2">
            <Button variant="gold" size="sm" className="w-full justify-start" onClick={() => onGo("schedule", "board")}>
              <Icon name="plus" size={14} /> New court booking
            </Button>
            <Button variant="ghost" size="sm" className="w-full justify-start" onClick={() => onGo("schedule", "requests")}>
              <Icon name="calendar-clock" size={14} /> Review requests
            </Button>
            {data ? (
              <>
                <Button variant="ghost" size="sm" className="w-full justify-start" onClick={onLogSale}>
                  <Icon name="receipt" size={14} /> Log sale
                </Button>
                <Button variant="ghost" size="sm" className="w-full justify-start" onClick={onAddClient}>
                  <Icon name="users" size={14} /> Add client
                </Button>
      ...
      {data ? <ActivityFeed data={data} /> : null}
    </div>
  );
}
...
function OverviewTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <ActivityFeed data={data} className="lg:col-span-2" />
...
function ActivityFeed({ data, className }: { data: AdminData; className?: string }) {
  const [showAll, setShowAll] = useState(false);
  const activity = data.activity;
  const shown = showAll ? activity : activity.slice(0, 10);

  return (
    <section className={cn("rounded-xl border border-border bg-surface-2 p-4", className)}>
      <h2 className="font-display text-sm font-extrabold uppercase tracking-wide text-foreground">Activity feed</h2>
          ...
          <li key={row.id} className="flex items-start gap-3 text-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground/60">
                {new Date(row.created_at ?? "").toLocaleTimeString("en-PH", { hour: "numeric", minute: "2-digit" })}
              </p>
            </div>
          </li>
        ))}
                      ...
                      return (
                        <div key={iso} className="border-l border-border/60 p-1">
                          ...
                          >
                            ...
                            >
                              {memberName(data, booking.member_id, booking.booker_name)}
                            </p>
                      ...
                      return (
                        <div key={iso} className="border-l border-border/60 p-1">
                          <div
                            ...
                            title={`${memberName(data, cont.member_id, cont.booker_name)} (continued)`}
                          />
                        </div>
                      );
                    }
...
function QuickBookForm({
  ...
  const maxHours = Math.min(8, 24 - hour); // createAdminBooking allows 1-8 hrs
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label={`Hours (max ${maxHours})`}>
          {/* Type any length, or step it with the arrows. */}
          <div className="flex items-center gap-3">
            <input
              type="number"
              inputMode="numeric"
              ...
              step={1}
              value={hours}
              onChange={(e) => setHours(Math.min(maxHours, Math.max(1, Math.round(Number(e.target.value)) || 1)))}
              className="fae-input w-24 text-center tabular-nums"
            />
            <span className="text-sm text-muted-foreground">
              hrs · {formatHour(hour)} to {formatHour(hour + hours)}
            </span>
          </div>
        </Field>
        <Field label="Status">
...
function RatesSettings() {
  ...
  const [draft, setDraft] = useState<Record<string, { member: number; nonMember: number }>>({});
  const [saving, setSaving] = useState<string | null>(null);

  const { data } = useQuery({ queryKey: ['court-rates'], queryFn: () => getCourtRates({ data: undefined }) });
  const rates = data?.rates ?? [];

  const valueFor = (r: (typeof rates)[number]) =>
    draft[r.court_id] ?? { member: Number(r.member_rate), nonMember: Number(r.non_member_rate) };

  async function save(courtId: string) {
    const v = draft[courtId];
    if (!v) return;
    setSaving(courtId);
    try {
      await updateCourtRate({ data: { courtId, memberRate: v.member, nonMemberRate: v.nonMember } });
      toast('Rate saved — the booking page charges this now.');
      setDraft((d) => { const n = { ...d }; delete n[courtId]; return n; });
      await qc.invalidateQueries({ queryKey: ['court-rates'] });
    } catch (err) {
      toast(err instanceof Error ? err.message : 'Could not save that rate.');
    } finally {
      setSaving(null);
    }
  }
  ...
  return (
    <Accordion icon="chart" title="Court rates" subtitle="Live prices — saving changes what customers are charged">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[560px] text-sm">
          ...
          <tbody>
            {rates.length === 0 ? (
              <tr><td colSpan={5} className="px-2 py-6 text-center text-muted-foreground">No rates configured.</td></tr>
            ) : (
              rates.map((r) => {
                const v = valueFor(r);
                const dirty = !!draft[r.court_id];
                const acc = SPORTS[r.sport as SportKey]?.acc ?? '#8a8a92';
                return (
                  <tr key={r.court_id} className="border-b border-border/60 last:border-b-0">
                    <td className="px-2 py-2">
                      <span className="flex items-center gap-2 text-muted-foreground">
                        <span className="h-2 w-2 rounded-sm" style={{ backgroundColor: acc }} />
                        {SPORTS[r.sport as SportKey]?.label ?? r.sport}
                      </span>
                    </td>
                    ...
                    <td className="px-2 py-2">
                      <input
                        type="number" min={0}
                        value={v.member}
                        onChange={(e) => setDraft({ ...draft, [r.court_id]: { ...v, member: Number(e.target.value) || 0 } })}
                    ...
                    <td className="px-2 py-2">
                      <input
                        type="number" min={0}
                        value={v.nonMember}
                        onChange={(e) => setDraft({ ...draft, [r.court_id]: { ...v, nonMember: Number(e.target.value) || 0 } })}
                        className="fae-input w-24 px-2 py-1 font-mono text-xs"
                      />
                    </td>
                    <td className="px-2 py-2">
                      {dirty ? (
                        <Button size="xs" variant="gold" disabled={saving === r.court_id} onClick={() => void save(r.court_id)}>
                          {saving === r.court_id ? 'Saving…' : 'Save'}
                        </Button>
                      ) : null}
                    </td>
                  </tr>
      ...
      <p className="mt-3 font-mono text-[10px] leading-relaxed text-muted-foreground">
        Stored in the database, not this browser. A saved rate applies to the next booking on any device, and every
